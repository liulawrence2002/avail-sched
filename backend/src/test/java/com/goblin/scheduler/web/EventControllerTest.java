...
class EventControllerTest {
  ...
  void join_returnsTokenAndParticipantLink() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/events/pub123/participants")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
  ...
  void updateAvailability_returns204() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/events/pub123/participants/tok789/availability")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
  ...
  void getParticipantAvailability_returnsSavedItems() throws Exception {
    ...
    mockMvc
        .perform(get("/api/events/pub123/participants/tok789/availability"))
        .andExpect(status().isOk())
  ...
  void getResults_returnsAggregateResponse() throws Exception {
    ...
    mockMvc
        .perform(get("/api/events/pub123/results"))
        .andExpect(status().isOk())
  ...
  void getResults_hostOnly_returnsForbidden() throws Exception {
    ...
    mockMvc.perform(get("/api/events/pub123/results")).andExpect(status().isForbidden());
  }
  ...
  void getHostResults_returnsNamedResponse() throws Exception {
    ...
    mockMvc
        .perform(get("/api/host/host456/results"))
        .andExpect(status().isOk())
  ...
  void finalizeEvent_returnsResponse() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/events/pub123/finalize?hostToken=host456")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
  ...
  void downloadIcs_returnsCalendarContentType() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            header()
                .string("Content-Disposition", "attachment; filename=\"goblin-scheduler.ics\""));
  }
}
