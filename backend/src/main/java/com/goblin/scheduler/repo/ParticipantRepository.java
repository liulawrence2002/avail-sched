...
public class ParticipantRepository {
    ...
    private final RowMapper<Participant> mapper = (rs, rowNum) -> new Participant(
        rs.getLong("id"),
        rs.getLong("event_id"),
        rs.getString("token"),
    ...
    public Optional<Participant> findByTokenAndEventId(String token, long eventId) {
        return jdbcTemplate.query("select * from participants where token = ? and event_id = ?", mapper, token, eventId).stream().findFirst();
    }
